...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="/">
        <xsl:variable name="rows"
            select="//smParamNode[@id='DwBaseinfoStaff']/smParamNode[not(@*)]" />
        <xsl:variable name="DwBaseinfoStaffCertRows"
            select="//smParamNode[@id='DwBaseinfoStaffCert']/smParamNode[not(@*)]" />
        <xsl:variable name="DwInfoStaffUnitRows"
            select="//smParamNode[@id='DwInfoStaffUnit']/smParamNode[not(@*)]" />
        <xsl:variable name="pageinfo" select="//smParamNode[@id='DwBaseinfoStaff']/smParamNode[@id='pageinfo']"/>
            <respInfo>
                 <resultconunt>
            ...
            <xsl:for-each select="$rows">
                <staffInfo>
                    ...
                    <xsl:if test="count($DwBaseinfoStaffCertRows) !=0">
                    <xsl:for-each select="$DwBaseinfoStaffCertRows">
                    <certificateInfo>
                        <workTypeCode>
                      ...
                      <xsl:if test="count($DwInfoStaffUnitRows) !=0">
                      <xsl:for-each select="$DwInfoStaffUnitRows">
                    <ownerInfo>
                        <ownerId>
