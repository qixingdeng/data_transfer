...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/">
		<xsl:variable name="rows"
			select="//smParamNode[@id='DwBaseinfoVehicle']/smParamNode[not(@*)]" />
		<xsl:variable name="pageinfo" select="//smParamNode[@id='DwBaseinfoVehicle']/smParamNode[@id='pageinfo']"/>
        	<respInfo>
		         <resultconunt>
			...
			<xsl:for-each select="$rows">
				<vehicleInfo>
					...
					<certificateInfo>
						<checkYear>
							<xsl:value-of select=".//smParamNode[@key='checkYear']/@value" />
						</checkYear>
						<checkState>
							<xsl:value-of select=".//smParamNode[@key='checkState']/@value" />
						</checkState>
						<checkDate>
							<xsl:value-of select=".//smParamNode[@key='checkDate']/@value" />
						</checkDate>
						<expireDate>
							<xsl:value-of select=".//smParamNode[@key='expireDate']/@value" />
						</expireDate>
						<nextDate>
							<xsl:value-of select=".//smParamNode[@key='nextDate']/@value" />
						</nextDate>
					</certificateInfo>
				</vehicleInfo>
			</xsl:for-each>
